#!/usr/bin/env python
# -*- coding:utf-8 -*-
# From https://github.com/Xtra-Computing/NIID-Bench
# A dataset util
...
import os.path
import logging
from demos.tools import utils_local as ul
...
def download_url(url: str, root: str, filename: Optional[str] = None, md5: Optional[str] = None) -> None:
    ...
    if check_integrity(fpath, md5):
        ...
        try:
            ...
            # Maybe you need proxy to retrieve file from github
...
class FEMNIST(MNIST):
    ...
    def _check_exists(self) -> bool:
        """
        重载MNIST的_check_exists，处理url转文件名 tar.gz 变 tar 的问题
        """
        return all(
    ...
    def download(self):
        ...
        ul.mkdir(self.raw_folder)
        ul.mkdir(self.processed_folder)
        ...
        print('Processing...')
        shutil.move(os.path.join(self.raw_folder, self.training_file), os.path.join(self.processed_folder, self.training_file))
        shutil.move(os.path.join(self.raw_folder, self.test_file), os.path.join(self.processed_folder, self.test_file))
