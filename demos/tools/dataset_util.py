...
from tools.datasets import MNIST_truncated, CIFAR10_truncated, SVHN_custom, FashionMNIST_truncated, CelebA_custom, FEMNIST
...
def pad_data(dataset, pdsz=None, add_channel=True, channel="nhwc"):
    """
    pad dataset images to specific shape
    """
    sp = dataset.shape
    if channel == 'nhwc':
        # for NHWC
        if add_channel:
            dataset = np.repeat(dataset[:, :, :, np.newaxis], 3, 3)
        if pdsz is not None:
            if sp[1] < pdsz:
                dataset = np.pad(dataset, ((0, 0), (0, pdsz - sp[1]), (0, 0), (0, 0)), 'constant')
            if sp[2] < pdsz:
                dataset = np.pad(dataset, ((0, 0), (0, 0), (0, pdsz - sp[2]), (0, 0)), 'constant')
    else:
        # for NCHW
        if add_channel:
            dataset = np.repeat(dataset[:, np.newaxis, :, :], 3, 1)
        if pdsz is not None:
            if sp[2] < pdsz:
                dataset = np.pad(dataset, ((0, 0), (0, 0), (0, pdsz - sp[2]), (0, 0)), 'constant')
            if sp[3] < pdsz:
                dataset = np.pad(dataset, ((0, 0), (0, 0), (0, 0), (0, pdsz - sp[3])), 'constant')
    return dataset